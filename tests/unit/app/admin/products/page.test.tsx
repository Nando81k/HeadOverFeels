...
vi.mock('@/components/admin/dashboard/AdminProductsV2', () => ({
  AdminProductsV2: () => <div data-testid="v2">V2 products</div>,
}))

vi.mock('@/components/admin/_v1/AdminProductsV1', () => ({
  AdminProductsV1: () => <div data-testid="v1">V1 products</div>,
}))
...
describe('admin/products/page dispatcher', () => {
  ...
  it('passes searchParams through to V2', async () => {
    ...
    vi.doMock('@/components/admin/dashboard/AdminProductsV2', () => ({
      AdminProductsV2: v2Spy,
    }))
    const { render, screen } = await import('@testing-library/react')
    const mod = await import('@/app/admin/products/page')
    const Page = mod.default
    render(await Page({ searchParams: Promise.resolve({ tab: 'collections' }) }))
    expect(screen.getByTestId('v2-with-params')).toBeInTheDocument()
    expect(v2Spy).toHaveBeenCalledWith(
      expect.objectContaining({ searchParams: { tab: 'collections' } }),
      undefined,
    )
  })
})
